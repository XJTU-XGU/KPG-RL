...
def gromov_wasserstein_sinkhorn(C1, C2, p, q, lam = 0.1, Mask = None,OT_algorithm="sinkhorn",max_iter_sinkhorn=2000,numItermax=5,fused=False,Cxy=None,alpha=0.5):
    ...
    t1 = time.time()
    pi = cg(p, q, C1, C2, constC, f, df, G0, lam=lam, Mask = Mask,OT_algorithm=OT_algorithm,max_iter_sinkhorn=max_iter_sinkhorn,numItermax=numItermax,fused=fused,Cxy=Cxy,alpha=alpha)
    t2 = time.time()
